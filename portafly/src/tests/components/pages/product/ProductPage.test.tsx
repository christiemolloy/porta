...
const setup = (asyncState: Partial<AsyncState<IProductOverview>>) => {
  ...
  const wrapper = render(<ProductPage productId={product.id.toString()} />)
  return wrapper
}
...
describe('when there is an error', () => {
  it('should show an alert', () => {
    const { getByText } = setup({ error: new Error('Not Found') })
...
describe('when the product is loaded', () => {
  it('should render properly', () => {
    ...
    expect(getByText('edit_link.title')).toBeInTheDocument()
    expect(getByText('delete_button.title')).toBeInTheDocument()
